...
class RecurrentMemNNCell(tf.contrib.rnn.RNNCell):
  def __init__(self, inputs, num_units, batch_size, dropout, mention_scores, antecedent_features, k):
    self._num_units = 1 + k
    self._dropout = dropout
    ...
    sen_emb = tf.reshape(tf.transpose(self.input_emb, [1, 0, 2])[0], [1, -1])
    init_word_emb = tf.zeros([1, tf.shape(self.input_emb)[2]])
    self.sentence_emb_init = tf.concat([init_word_emb, self.span_emb], 0) # [num_words, emb]
    self.hist_shape = tf.shape(self.sentence_emb_init)
    ...
    initial_memory = tf.concat([initial_index, A, sen_emb, self.padding], 1)
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with tf.variable_scope(scope or type(self).__name__):  # "RecurrentMemNNCell"
      ...
      pair_span_emb = tf.concat([pair_feature, inputs_tiled, self.span_emb], 1)

      # mention_att = projection_name(pair_emb, 1, 'context_att') + self.mention_scores + context_mention_score
      mention_att = ffnn_name(pair_span_emb, 2, 150, 1, 'context_att', self._dropout) + self.mention_scores + context_mention_score


      word_mask = tf.reshape(tf.sequence_mask(word_index + 1, self.k + 1, dtype=tf.float32), [-1, 1])
      ...
      antecedent_scores = tf.concat([tf.zeros([1, 1]), mention_att], 0) + tf.log(word_mask)
      # top_antecedent = tf.argmax(tf.transpose(mention_att), axis=1)
      # hist_emb = tf.gather(self.sentence_emb_init, top_antecedent)

      # '''
      hist_attn = tf.nn.softmax(antecedent_scores, dim=0)
      ...
      hist_emb = tf.tile(hist_emb, [self.k, 1])
      # '''

      # sentence_emb = tf.reshape(memories, [self.k, self.hidden_size])
      pair_entity_emb = tf.concat([pair_feature, inputs_tiled, sentence_emb, hist_emb], 1)
      entity_att = .8 * (ffnn_name(pair_entity_emb, 2, 150, 1, 'entity_att', self._dropout) +\
              self.mention_scores + context_mention_score)
      entity_scores = tf.concat([tf.zeros([1, 1]), entity_att], 0) + tf.log(word_mask) + .2 * antecedent_scores
      
      top_antecedent = tf.argmax(tf.transpose(entity_scores), axis=1)
      ...
      new_h = tf.transpose(entity_scores)

...
class CorefModel(object):
  ...
  def get_predictions_and_loss(self, word_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids):
    ...
    context_pre_starts = tf.maximum(mention_starts - 10, 0)
    context_pos_ends = tf.minimum(mention_ends + 10, util.shape(text_outputs, 0) - 1)
    ...
    antecedent_scores = self.get_antecedent_scores(mention_emb,
                                                    ...
                                                    text_outputs,
                                                    context_pre_starts,
                                                    context_pos_ends) # [num_mentions, max_ant + 1]
  ...
  def get_mention_emb(self, text_emb, text_outputs, mention_starts, mention_ends):
    ...
    mention_output = tf.tile(tf.expand_dims(mention_output, 1), [1, self.config["max_mention_width"], 1])

    mention_output_score = util.projection_name(mention_output, 1, "mention_output_score")
    ...
    if self.config["model_heads"]:
      ...
      mention_head_scores = tf.gather(self.head_scores, mention_indices) + mention_output_score # [num_mentions, max_mention_width, 1]
  ...
  def get_antecedent_scores(self,
    ...
    context_output = tf.tile(tf.expand_dims(context_output, 1), [1, self.config["max_context_width"], 1])

    mention_output = tf.tile(tf.expand_dims(mention_emb, 1), [1, self.config["max_context_width"], 1])
    ...
    context_att_score = util.projection_name(tf.concat([context_glove_emb, context_output, mention_output], 2), 1, "context_att")

    context_attention = tf.nn.softmax(context_att_score + tf.log(context_mask), dim=1) # [num_mentions, max_mention_width, 1]

    context_emb = tf.reduce_sum(context_attention * context_glove_emb, 1) # [num_mentions, emb]

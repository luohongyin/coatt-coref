...
.Input("max_context_width: int32")
.Output("output_mention_indices: int32")
.Output("context_starts: int32")
.Output("context_ends: int32")
.Output("context_length: int32");
...
public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    int max_context_width = context->input(4).scalar<int32>()(0);
    ...
    Tensor* context_starts_tensor = nullptr;
    //TensorShape output_mention_indices_shape({num_output_mentions});
    OP_REQUIRES_OK(context, context->allocate_output(0, output_mention_indices_shape, &context_starts_tensor));
    TTypes<int32>::Vec context_starts = context_starts_tensor->vec<int32>();

    Tensor* context_ends_tensor = nullptr;
    //TensorShape output_mention_indices_shape({num_output_mentions});
    OP_REQUIRES_OK(context, context->allocate_output(0, output_mention_indices_shape, &context_ends_tensor));
    TTypes<int32>::Vec context_ends = context_ends_tensor->vec<int32>();

    Tensor* context_length_tensor = nullptr;
    //TensorShape output_mention_indices_shape({num_output_mentions});
    OP_REQUIRES_OK(context, context->allocate_output(0, output_mention_indices_shape, &context_length_tensor));
    TTypes<int32>::Vec context_length = context_length_tensor->vec<int32>();
    ...
    for (int i = 0; i < num_output_mentions; ++i) {
      ...
      if (i == 0) {
        context_starts(i) = candidate_starts(top_mention_indices[i]);
      } else {
        context_starts(i) = candidate_starts(top_mention_indices[i - 1]);
      }
      if (i == num_output_mentions - 1) {
        context_ends(i) = candidate_ends(top_mention_indices[i]);
      } else {
        context_ends(i) = candidate_ends(top_mention_indices[i + 1]);
      }
      if (context_ends(i) - context_starts(i) + 1 > max_context_width) {
        int exceed = context_ends(i) - context_starts(i) + 1 - max_context_width;
        if (i == 0) context_ends(i) -= exceed;
        if (i == num_output_mentions - 1) context_starts(i) += exceed;
        if (i > 0 && i < num_output_mentions - 1) {
          int s = candidate_starts(top_mention_indices[i]);
          int e = candidate_ends(top_mention_indices[i]);
          int len = e - s + 1;
          int expand = int((max_context_width - len) * 0.5);
          context_starts(i) = std::max(context_starts(i) - expand, 0);
          context_ends(i) = std::min(context_ends(i) + expand, candidate_ends(top_mention_indices[i + 1]));
        }
      }
      context_length(i) = context_ends(i) - context_starts(i) + 1;
    }
  }
private:
